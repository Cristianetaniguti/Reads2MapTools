...
# * Pirs diploid simulated SNPs and indels
#
...
#' @param ref.map data.frame with information from reference linkage maps within columns bp and cm
#' @param seed integer defining the simulation seed
#' @param P1 parent1 ID in VCF file
#' @param P2 parent2 ID in VCF file
...
create_haplo <- function(vcfR.obj, ref.map, seed, P1, P2, filename = "founders.txt"){
  ...
  ref.alleles <- vcfR.obj@fix[,4]
  alt.alleles <- vcfR.obj@fix[,5]
  mk.names <- unique(paste0(vcfR.obj@fix[,1], "_", vcfR.obj@fix[,2]))
...
#'
#' @param ref.map data.frame with columns bp and cM
...
#'
#' @param seed integer defining the simulation seed
#' @param popsize integer defining the population size
#' @param filename a character with the parfile name
...
create_parfile <- function(seed, popsize, filename = "parameters.txt",
                           ...
                           name.founderfile = "founderfile.txt",
                           name.chromfile= "chromosome.txt",
                           name.out = "sim"){
...
#' Codifying phases according with GUSMaap
#' for comparisons
#'
#' @param founderfile input file for PedigreeSim software
#' @param ref_alt_alleles data.frame with chromosome, position, reference allele,
#' alternative allele, position in centimorgan information
#'
#'@export
